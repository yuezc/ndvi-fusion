...
[![Code: MIT](https://img.shields.io/badge/Code-MIT-green.svg)](LICENSE) [![Data: CC BY 4.0](https://img.shields.io/badge/Data-CC%20BY%204.0-blue.svg)](<TBD: dataset URL>) [![Paper](https://img.shields.io/badge/Paper-TBD-lightgrey.svg)](<TBD: paper URL>)
...
| Spatial resolution | `<TBD>`; the synthetic generator uses 0.25° AVHRR and 0.10° MODIS demonstration grids, while preprocessing aligns MODIS to the AVHRR grid |
...
| CRS | EPSG:4326 in the synthetic generator; final input/output CRS is inherited from the AVHRR profile (`<TBD>` for production data) |
| Scale factor | `<TBD>`; no explicit scale-factor conversion is present in the five scripts |
| NoData | `<TBD>` for production data; the synthetic generator uses `src.config.NODATA_VALUE` |
| Product type | Long-term harmonized / fused NDVI time series |

The local `raw/` and `NDVI/` directories contain about 4.2 GiB of raster data and are excluded from this repository. The repository retains empty directory placeholders. Obtain the production dataset from: **`<TBD: dataset URL>`**.
...
- `raw/` — placeholder for source AVHRR/MODIS or intermediate input data; large rasters are intentionally omitted.
- `NDVI/` — placeholder for corrected, fused, or final NDVI products; large rasters are intentionally omitted.
...
- Dataset: CC BY 4.0.
- Paper: `<TBD: DOI or public paper link>`.
...
- Exact semantics and provenance of the individual `NDVI/YYYYMM.tif` files.
